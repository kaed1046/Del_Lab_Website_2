...
function loadStatsPage(){
	var table = document.getElementById("stats_table");//Retrieve table elements
	var row_counter;//Keeps track of row index
	var home = "Cu Boulder";
	var away = "";
	var wins = 0;
	var lose = 0;
	...
	{//Outer for loop iterates over each row
		away = table.rows[row_counter].cells[1].innerHTML;
		if(table.rows[row_counter].cells[2].innerHTML >  table.rows[row_counter].cells[3].innerHTML)
		{
			...
			table.rows[row_counter].cells[4].innerHTML = away;
			lose++;
		}
	}
...
function loadPlayersPage()
{
	...
	for(row_counter = 0; row_counter < players.length; row_counter++)
	{
		...
		x.href = "#";
		x.setAttribute("onclick", "switchPlayers(" + row_counter +")");
		x.setAttribute("class", "dropdown-item");
		x.innerText = players[row_counter].name;
		drop.appendChild(x);
	}
}
...
function switchPlayers(playerNum){
	...
	document.getElementById("ave_p_yards").innerHTML = players[playerNum].pass_yards/players[playerNum].games_played;
	document.getElementById("ave_r_yards").innerHTML = players[playerNum].rushing_yards/players[playerNum].games_played;
	document.getElementById("ave_rec_yards").innerHTML = players[playerNum].receiving_yards/players[playerNum].games_played;
}
/*
